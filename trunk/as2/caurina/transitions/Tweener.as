...
class caurina.transitions.Tweener {
	...
	public static function removeTweensByTime (p_scope:Object, p_properties:Object, p_timeStart:Number, p_timeComplete:Number):Boolean {
		...
		for (j = 0; j < _tweenList.length; j++) {
			if (_tweenList[j] != undefined && _tweenList[j].properties != undefined && _tweenList[j].properties.length > 0 && p_scope == _tweenList[j].scope) {
	...
	public static function pauseTweens (p_scope:Object):Boolean {
		...
		var i:Number;
		for (i = 0; i<_tweenList.length; i++) {
			if (_tweenList[i].scope == p_scope && !_tweenList[i].isPaused) {
				pauseTweenByIndex(i);
				paused = true;
			}
		}
	 ...
	 * @return							Boolean		Whether or not it successfully removed this tweening
	 */
	public static function removeTweenByIndex (i:Number, p_finalRemoval:Boolean):Boolean {
	...
	public static function debug_getList():String {
		...
		for (i = 0; i<_tweenList.length; i++) {
//			if (_tweenList[i].scope == _level0.desktop.trilhaPlayer.snd) {
				ttl += "["+i+"] ::\n";
